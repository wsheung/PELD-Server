...
def update_fleet_metadata(current_user, client=False):
    ...
    fleet_data = esi.get_character_fleet(current_user.get_id(), current_user.access_token)

    data_to_update['id'] = fleet_data['fleet_id']
    data_to_update['fc_id'] = fleet_data['fleet_boss_id']
    current_user.set_fleet_id(fleet_data['fleet_id'])
    current_user.set_fleet_role(fleet_data['role'])
...
def get_fleet_members(current_user, fleet_doc):
    update_token(current_user)
    members = esi.get_fleet_members(current_user.fleet_id, current_user.access_token)
    new_members = [member['character_id'] for member in members]

    data_to_update = {}
    # audit connected sockets to ensure they are still in fleet
    ...
    return members, data_to_update['connected_clients']

def get_fleet_wings(current_user):
    update_token(current_user)
    wings = esi.get_fleet_wings(current_user.fleet_id, current_user.access_token)
    wings.sort(key=lambda x: x['id'])
    for wing in wings:
